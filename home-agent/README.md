...
| かわいい | 明るくて人懐っこい口調が既定。落ち着いた口調にも切り替えられる |
| VTuber みたいなアバター | まばたき・呼吸・視線追従つきのキャラクターが画面にいて、内面に合わせて表情が変わり、しゃべるときは台詞のかなを読んで口が動く |
...
今日のタスク・アバター・さっき言ったこと・日記がひとまとめに出て、そこから話しかけられます。

## アバター

![ひなたの表情](docs/faces.png)

画面のひなたは、外部のモデルファイルもライブラリも使わずその場で描いている SVG のリグです
（`web/avatar.js`。依存ゼロ・完全オフライン・約 400 行）。

- **表情は内面から自動で決まる**: ごきげん / ふつう / しょんぼり / ふてくされ（汗）/ ねむい（zzz）/ さみしい。
  苛立ちが溜まれば勝手に不機嫌な顔になり、夜は眠そうになります。
- **リップシンク**: 台詞のかなを母音（あいうえおん）に変換して口を動かします。伸ばし棒は直前の母音を伸ばし、
  句読点では閉じます。読み上げ（Web Speech / VOICEVOX）と同時に動きます。
- **生きている感じ**: まばたき（間隔はランダム）、呼吸で肩が上下、アホ毛が揺れる、カーソルや指を目で追う。
- **つつくと反応**: アバターをタップすると驚いて跳ねます。
- `prefers-reduced-motion` が有効な環境では、揺れを止めて表情だけ変えます。

### ホーム画面に置く

`manifest.webmanifest` とアイコンを同梱しているので、スマホのブラウザで「ホーム画面に追加」すると
アドレスバー無しのアプリとして開きます。

### Live2D / VRM に差し替える

`createAvatar()` は次の 4 つを持つオブジェクトを返すだけの約束になっています。
同じ形のものを返す実装を書けば、そのまま差し替わります。

```js
avatar.setExpression('sulky');  // happy / normal / sad / sulky / sleepy / lonely / surprise
avatar.speak('ゴミ出しの時間だよ');
avatar.stopSpeaking();
avatar.poke();
```

- **VRM（VRoid Studio で作った .vrm）**: `@pixiv/three-vrm` + three.js で読み込み、
  表情は `vrm.expressionManager.setValue('happy' | 'sad' | 'angry' | 'relaxed', 1)`、
  口は `moraShapes()` の戻り値をそのまま viseme に流せます（`a→aa` `i→ih` `u→ou` `e→ee` `o→oh` `n→閉じる`）。
- **Live2D**: Cubism SDK for Web + `.moc3` モデル。パラメータ `ParamEyeLOpen` / `ParamMouthOpenY` /
  `ParamAngleZ` に、このリグと同じ値（まぶたの開き・口の開き・首の傾き）を渡せます。

どちらもモデルファイルとライセンスは各自で用意してください（この同梱アバターは外部素材を使っていません）。
...
```bash
npm test     # 61 件（スケジューラ、自我、口調、耳と口、アバター、会話、統合）
npm run demo # 一日を早送り
```
